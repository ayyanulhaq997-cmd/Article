...
import { Search, Tag, ChevronRight, Globe, ShieldAlert, Loader2 } from 'lucide-react';
...
const BlogPage = () => {
  ...
  useEffect(() => {
    const loadArticles = async () => {
      ...
      // Merge: Hardcoded first, then Cloud
      const combined = [...ARTICLES];
      
      // Prevent duplicates if hardcoded IDs match cloud IDs
      cloudArticles.forEach(ca => {
        if (!combined.find(a => a.id === ca.id)) {
          combined.push(ca);
        }
      });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-32">
            ...
            <p className="text-slate-500 font-bold">Synchronizing with Cloud Database...</p>
          </div>
        ) : (
              ...
              <div key={article.id} className="group bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 flex flex-col">
                <div className="relative h-60 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 flex flex-wrap gap-2">
                    <span className="px-3 py-1 bg-white/95 backdrop-blur-sm text-blue-600 text-[10px] font-black rounded-lg uppercase tracking-wider shadow-sm border border-slate-100">
                      {article.category}
                    </span>
